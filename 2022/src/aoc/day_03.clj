...
(sum (map split-sack (file->lines "input/day03.example")))
(sum (map split-sack (file->lines "input/day03.input")))

(defn partition-sack [lines]
  (for [group (partition 3 lines)]
    (score (first (apply set/intersection (map set group))))))

(sum (partition-sack (file->lines "input/day03.example")))
(sum (partition-sack (file->lines "input/day03.input")))
